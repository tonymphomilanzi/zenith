...
const Navbar = () => {
  ...
  useGSAP(() => {
    ...
    // FIXED: We use .to() instead of .from()
    // This prevents the "Flash" because the element starts hidden in CSS
    ...
    .to(logoRef.current, {
      opacity: 1,
      letterSpacing: "0.3em", // Animate spacing OUT
      duration: 1.5,
      ease: "power3.out"
    }, "-=1");
  ...
  return (
    <nav 
      ref={navRef} 
      // 1. ADDED 'opacity-0' and '-translate-y-full' to hide it initially
    ...
    >
      ...
      <div className="absolute left-1/2 -translate-x-1/2 text-center">
        <h1 
          ref={logoRef} 
          // 2. ADDED 'opacity-0' and small 'tracking' to animate slightly
      ...
      <div className="flex gap-8 items-center">
        ...
        <Search className="w-5 h-5 cursor-pointer hover:scale-125 transition-transform duration-300" />
      </div>
    </nav>
  );
};
